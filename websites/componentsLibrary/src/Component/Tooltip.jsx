...
export default function Tooltip({children, color='primary', theme, title='Tooltip'}) {
  return (              
        <span className={`tooltip-target `} >
            {children}
            <span className={`tooltip-text tooltip-${theme || 'dark'}`} style={{
                backgroundColor: styles[color]?.backgroundColor || styles['primary'].backgroundColor,
                color: styles[color]?.color || styles['primary'].color,
                border: styles[color]?.border || styles['primary'].border
                }}> 
